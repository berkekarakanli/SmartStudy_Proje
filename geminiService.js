...
async function generateHomeworkPlan({ sinif, sinavTuru, aytAlani, hedef, tamamlananKonular, zayifKonular, tekrarEdenZayifKonular, izinliMufredat, sinavTarihi, kalanGun, sonAnalizler, hataDefteriDersSayilari, haftalikHedefSoru, ortalamaNetYuzdesi, sonAnalizTuru }) {
    ...
    try {
        ...
        const aytKapsiyor = /^AYT/.test(sinavTuru || '');
...
${sureBaskisiKurali}
- HAFTALIK TOPLAM SORU HEDEFİ: Bu hafta için TÜM derslerin/satırların TOPLAMI yaklaşık ${haftalikHedefSoru} soru olacak şekilde plan yap (bu sayı öğrencinin net başarı geçmişine göre önceden hesaplandı, ciddi bir YKS/KPSS adayının haftalık hacmini yansıtıyor). "Her gün" etiketli bir satır haftanın 7 gününe uygulanacağı için o satırın soru_sayisi'si × 7 bu toplama dahil olur - bunu hesaba katarak dağıt (örn. "Her gün" etiketli bir satırda soru_sayisi=20 ise bu satır tek başına haftalık 140 soru demektir). Öğrencinin gerçekten bu hacme hazır olmadığını düşünüyorsan (deneme geçmişi çok az/düzensizse ya da hedefinde/sohbetinde açık bir zaman kısıtı belirtilmişse) bu hedefi makul ölçüde aşağı çekebilirsin, ama keyfi şekilde çok düşük bırakma - amaç gerçek bir sınav temposu, rahat bir tekrar programı değil.
