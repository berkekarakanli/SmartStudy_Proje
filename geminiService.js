 ...
 */
async function generateHomeworkPlan({ sinif, sinavTuru, aytAlani, hedef, tamamlananKonular, zayifKonular, tekrarEdenZayifKonular, izinliMufredat, sinavTarihi, kalanGun, sonAnalizler, hataDefteriDersSayilari, haftalikHedefSoru, ortalamaNetYuzdesi, sonAnalizTuru }) {
    if (!ai) return null;
    ...
    try {
        ...
        // Öğrenci bir analiz TYT için yapıp sınıf/alan bilgisine göre AYT
        // konuları da izinli müfredata giriyorsa, model bunu fark etmeden
        // otomatik AYT ödevi verebiliyordu (öğrencinin şikayet ettiği bir
        // durum) - bunu açıkça işaretleyip öğrenciye SORMASINI istiyoruz.
        const aytKapsiyor = sinavTuru === 'TYT+AYT';
        const aytUyumsuzlukUyarisi = (aytKapsiyor && sonAnalizTuru === 'TYT')
            ? `\n- DİKKAT - KAPSAM UYUMSUZLUĞU: Öğrencinin en son kaydettiği net analizi SADECE TYT için, AYT için değil - ama sınıf/alan bilgisine göre bu hafta planı AYT konularını da kapsayabilir. Bu hafta AYT konularına ağırlık VERME, önceliği en son TYT analizindeki zayıf derslere ver. "deneme_onerisi" alanına öğrenciye şunu soran kısa, doğal bir not ekle: son analizini sadece TYT için yaptığını fark ettiğini, bu hafta sadece TYT'ye mi odaklanmak istediğini yoksa AYT'yi de plana katmasını mı istediğini, AYT istiyorsa konu+soru bazlı mı yoksa deneme çözmeye ağırlık vererek mi ilerlemek istediğini sor.`
            : '';

        const tamamlananVarMi = Object.values(tamamlananKonular || {}).some(arr => Array.isArray(arr) && arr.length > 0);
        const gercektenBaslangicSeviyesi = !tamamlananVarMi && (ortalamaNetYuzdesi === null || ortalamaNetYuzdesi < 15);
...
- BAŞLANGIÇ NOKTASI KARARI: ${gercektenBaslangicSeviyesi
        ? 'Öğrenci GERÇEKTEN sıfır seviyede görünüyor (hiç tamamlanan konu bildirmemiş VE net başarısı çok düşük ya da hiç veri yok) - bu durumda müfredatı BAŞTAN, doğal/mantıklı bir sırayla (temel/giriş konularından başlayarak) planla.'
        : 'Öğrenci sıfır seviyede DEĞİL (ya bazı konuları bitirmiş ya da ölçülebilir bir net başarısı var) - bu yüzden müfredatı baştan sırayla İŞLEMEYE KALKMA. Onun yerine SADECE yanlış/boş çıkan, zayıf ya da tekrarlayan konulara odaklan: hangi derste/konuda en çok yanlış yapmış ya da en çok boş bırakmışsa oraya öncelik ver, zaten iyi olduğu temel konuları tekrar "öğret" gibi davranma.'}${aytUyumsuzlukUyarisi}
...
${sureBaskisiKurali}
- HAFTA SONU (Cumartesi/Pazar) SENİN İŞİN DEĞİL: Sistem, hafta sonu için otomatik olarak deneme sınavı + hata analizi görevi ekliyor (sen bunu yazmıyorsun). SEN SADECE Pazartesi-Cuma (5 gün) için plan yap - "gun" alanına KESİNLİKLE "Cumartesi" ya da "Pazar" yazma, bu değerler artık geçersiz. Ayrıca Paragraf/Problem/Geometri gibi sabit günlük beceri pratiğini de sen ekleme - onu da sistem otomatik ekliyor, sen SADECE öğrencinin zayıf/tekrarlayan/yeni öğrenmesi gereken KONULARA odaklanan satırlar üret.
- HAFTA İÇİ (Pazartesi-Cuma) TOPLAM SORU HEDEFİ: Bu 5 günün TÜM derslerin/satırların TOPLAMI yaklaşık ${haftalikHedefSoru} soru olacak şekilde plan yap (bu sayı öğrencinin sınav türüne ve net başarı geçmişine göre önceden hesaplandı - sistemin ayrıca eklediği sabit pratik ve hafta sonu deneme hariç, SADECE senin bu 5 gün için ürettiğin satırların payı). "Her gün" etiketli bir satır artık SADECE bu 5 güne uygulanır, o yüzden o satırın soru_sayisi'si × 5 bu toplama dahil olur (7 değil). Öğrencinin gerçekten bu hacme hazır olmadığını düşünüyorsan (deneme geçmişi çok az/düzensizse ya da hedefinde/sohbetinde açık bir zaman kısıtı belirtilmişse) bu hedefi makul ölçüde aşağı çekebilirsin, ama keyfi şekilde çok düşük bırakma - amaç gerçek bir sınav temposu, rahat bir tekrar programı değil.
- SORU SAYISINI KONUYA GÖRE BELİRLE, DÜZ SABİT SAYI YAPIŞTIRMA: soru_sayisi 5 ile 200 arasında olabilir, ama her konuya aynı sayıyı verme - bir rehber öğretmenin/koçun yapacağı gibi konunun GERÇEK niteliğine göre karar ver: (1) Sınavda SIK soru çıkan, BECERİ/pratik ağırlıklı konular (Paragraf, Problemler, Geometri, Fonksiyonlar, Kimyasal Hesaplamalar gibi) çok tekrar gerektirir - bunlara haftalık 60-200 soru aralığında geniş bir hacim ver. (2) Orta yoğunlukta, hem bilgi hem işlem gerektiren konular (Olasılık, Elektrik Akımı gibi) için 30-80 soru aralığı makul. (3) Ağırlıklı EZBER/bilgi konuları, sınavda az soru çıkan ya da tek seferde kavranabilen konular (bir tarih dönemi, bir coğrafya alt başlığı gibi) için 10-30 soru yeterli, buraya da 100 soru yığmak zaman israfıdır. Kısacası: konu ne kadar çok pratik/tekrar gerektiriyorsa o kadar yüksek, ne kadar "bir kere anlaşılıp geçilebilir" ise o kadar düşük soru_sayisi ver - amaç gerçekçi bir kaynak/soru bankası kullanım hacmi, rastgele yuvarlak bir sayı değil.
...
- Bir ödev satırındaki TÜM konular o satırın "ders" alanıyla AYNI derse ait olmalı - örneğin "ders":"Türkçe / Türk Dili ve Edebiyatı" olan bir satıra Matematik/Geometri konusu KOYMA, farklı derslerin konularını asla tek satırda birleştirme.
...
- HAFTA İÇİ PROGRAM ÜRET: Her ödev satırına hangi güne ait olduğunu belirten bir "gun" alanı ekle. Konu bazlı (yeni öğrenme/tekrar) görevleri FARKLI günlere dağıt (aynı günde 1-2 dersten fazlasını yığma, öğrenci her gün tek bir şeye boğulmasın). "gun" alanı SADECE şu değerlerden biri olabilir: "Pazartesi", "Salı", "Çarşamba", "Perşembe", "Cuma", "Her gün" (Her gün = sadece bu 5 gün, Cumartesi/Pazar DEĞİL).
